    ...
    def setup_model(self):
        quant_config = BitsAndBytesConfig(
            ...
            bnb_4bit_compute_dtype=getattr(
                torch, self.cfg.model.labeling.quantization.bnb_4bit_compute_dtype
            ),
        )
    ...
    def generate_prompt(self, review, few_shot=True):
        ...
        if few_shot:
            for ex in self.few_shot_examples:
                user_content = (
                    self.cfg.model.labeling.prompt_templates.user_template.format(
                        review=ex["review"]
                    )
                )
                assistant_content = (
                    self.cfg.model.labeling.prompt_templates.assistant_template.format(
        ...
        user_content = self.cfg.model.labeling.prompt_templates.user_template.format(
            review=review
        )
        messages.append({"role": "user", "content": user_content})
